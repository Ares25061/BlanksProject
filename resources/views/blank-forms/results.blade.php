...
<html lang="ru">
<head>
    @include('layouts.head', ['title' => 'Результаты проверки'])
...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div id="loading" class="text-center py-12">
        ...
        <p class="text-slate-600 mt-4 dark:text-slate-300">Загружаю результаты проверки...</p>
    </div>
        ...
        <section class="bg-white border border-slate-200 rounded-3xl shadow-sm p-6 dark:bg-slate-900 dark:border-slate-800 dark:shadow-none">
            <div class="flex flex-wrap justify-between items-start gap-4">
                <div>
                    ...
                    <p id="pageSubtitle" class="text-slate-600 mt-3 dark:text-slate-400">Показываю выбранные и правильные варианты ответов, а также итоговую оценку.</p>
                </div>
...
<script>
    ...
    async function loadResults() {
        if (!blankFormIds.length && !previewTokens.length) {
            ...
            document.getElementById('resultsList').innerHTML = `
                <section class="bg-white border border-dashed border-slate-300 rounded-3xl p-12 text-center text-slate-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-400">
    ...
    function renderResults(payloads) {
        document.getElementById('resultsList').innerHTML = payloads.map(({ data: blankForm, grade, scanPreviewUrls }) => {
            ...
            return `
                <section class="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800 dark:shadow-none">
                    <div class="p-6 border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950/70">
                        <div class="flex flex-wrap justify-between gap-4">
                            <div>
                                ...
                                <div class="text-slate-500 mt-2 dark:text-slate-400">${escapeHtml(blankForm.group_name || 'Группа не указана')} • Вариант ${escapeHtml(String(variantNumber))} • ${escapeHtml(blankForm.form_number || '')}</div>
                                ${blankForm.is_foreign_scan ? '<div class="mt-2 inline-flex items-center gap-2 rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold text-white dark:border dark:border-slate-700 dark:bg-slate-950">Чужой бланк • только OCR-разбор</div>' : ''}
                            </div>
                            <div class="grid sm:grid-cols-3 gap-3 min-w-[280px]">
                                <div class="bg-white border border-slate-200 rounded-2xl px-4 py-3 dark:bg-slate-950 dark:border-slate-700">
                                ...
                                <div class="bg-white border border-slate-200 rounded-2xl px-4 py-3 dark:bg-slate-950 dark:border-slate-700">
                                    <div class="text-xs uppercase tracking-[0.25em] text-slate-400 dark:text-slate-500">Оценка</div>
                                    <div class="text-xl font-bold mt-2">${escapeHtml(grade?.grade || blankForm.grade_label || '—')}</div>
                                </div>
                                <div class="bg-white border border-slate-200 rounded-2xl px-4 py-3 dark:bg-slate-950 dark:border-slate-700">
                                    <div class="text-xs uppercase tracking-[0.25em] text-slate-400 dark:text-slate-500">Процент</div>
                    ...
                    ${(scanPreviewUrls.length || recognizedSummary) ? `
                        <div class="p-6 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
                            <div class="grid xl:grid-cols-[0.95fr_1.05fr] gap-6 items-start">
                                <div>
                                    <div class="text-sm uppercase tracking-[0.25em] text-slate-400 mb-3 dark:text-slate-500">Скан бланка</div>
                                    ${scanPreviewUrls.length ? `
                                        <div class="space-y-4">
                                            ${scanPreviewUrls.map((page) => `
                                                <div class="rounded-3xl overflow-hidden border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-950/70">
                                                    <div class="px-4 py-3 border-b border-slate-200 bg-white text-sm font-medium text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">${escapeHtml(page.label)}</div>
                                                    <img src="${page.url}" alt="Скан бланка ${escapeHtml(studentName)} стр. ${page.pageNumber}" class="w-full h-auto block">
                                                </div>
                                            `).join('')}
                                        </div>
                                    ` : `
                                        <div class="rounded-3xl border border-dashed border-slate-300 text-slate-500 p-8 text-center dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
                                ...
                                <div class="space-y-4">
                                    <div class="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-950/70">
                                        <div class="text-sm uppercase tracking-[0.25em] text-slate-400 mb-3 dark:text-slate-500">Распознано со скана</div>
                                        <div class="font-semibold text-slate-900 dark:text-white">${recognizedSummary || 'Данные распознавания не сохранены.'}</div>
                                    </div>
                                    <div class="rounded-3xl border border-slate-200 bg-white p-5 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
                    ...
                    ${scanWarnings.length ? `
                        <div class="px-6 py-4 border-b border-amber-200 bg-amber-50 text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
                            ...
                            return `
                                <article class="p-6">
                                    <div class="flex flex-wrap justify-between gap-4 mb-4">
                                        <div>
                                            <div class="text-sm text-slate-500 dark:text-slate-400">Вопрос ${index + 1}</div>
                                            <h3 class="text-lg font-semibold text-slate-900 mt-1 dark:text-white">${escapeHtml(question.question_text)}</h3>
                                        </div>
                                        <div class="flex items-start gap-3">
                                            <span class="px-3 py-1 rounded-full text-sm ${questionState.className}">
                                                ${questionState.label}
                                            </span>
                                            <span class="px-3 py-1 rounded-full bg-slate-100 text-slate-700 text-sm dark:bg-slate-800 dark:text-slate-200">
                                    ...
                                    <div class="grid lg:grid-cols-[1fr_1fr] gap-4 mb-4">
                                        <div class="rounded-2xl border border-slate-200 p-4 bg-slate-50 dark:border-slate-700 dark:bg-slate-950/70">
                                            <div class="text-xs uppercase tracking-[0.25em] text-slate-400 mb-2 dark:text-slate-500">Выбранный вариант</div>
                                            <div class="font-semibold text-slate-900 dark:text-white">${selectedLabel}</div>
                                        </div>
                                        <div class="rounded-2xl border border-emerald-200 p-4 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-950/20">
                                            <div class="text-xs uppercase tracking-[0.25em] text-emerald-700 mb-2 dark:text-emerald-300">Правильный вариант</div>
                                            <div class="font-semibold text-emerald-900 dark:text-emerald-200">${correctLabel}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid gap-2">
                                        ${answers.map((answer) => {
                                            ...
                                            return `
                                                <div class="rounded-2xl border px-4 py-3 flex flex-wrap justify-between gap-3 ${isCorrect ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-950/20' : 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950'}">
                                                    <div class="flex items-start gap-3">
                                                        <span class="font-semibold">${escapeHtml(getAnswerLetter(answer, answers))}.</span>
                                                        <span>${escapeHtml(answer.answer_text)}</span>
                                                    </div>
                                                    <div class="flex items-center gap-2 text-sm">
                                                        ${isSelected ? '<span class="px-2 py-1 rounded-full bg-sky-100 text-sky-700 dark:bg-sky-500/15 dark:text-sky-300">Выбран</span>' : ''}
                                                        ${isCorrect ? '<span class="px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">Правильный</span>' : ''}
                                                    </div>
                                                </div>
                                            `;
                                        }).join('')}
                    ...
                    <div class="px-6 py-4 border-t border-slate-200 bg-slate-50 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-950/70 dark:text-slate-400">
    ...
    function renderAssignedGradePanel(blankForm, grade) {
        if (blankForm.is_foreign_scan) {
            return `
                <div class="rounded-3xl border border-dashed border-slate-300 bg-white p-5 text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
        ...
        if (!blankForm.group_student_id) {
            return `
                <div class="flex flex-wrap justify-between gap-4 items-start">
                    <div class="rounded-3xl border border-dashed border-slate-300 bg-white p-5 text-slate-500 flex-1 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
        ...
        const currentGrade = blankForm.assigned_grade_value
            ? `<div class="text-sm text-slate-500 mt-3 dark:text-slate-400">Сейчас в журнале: <span class="font-semibold text-slate-900 dark:text-white">${escapeHtml(blankForm.assigned_grade_value)}</span> от ${formatDate(blankForm.assigned_grade_date)}${blankForm.grade_assigner ? ` • поставил(а) ${escapeHtml(blankForm.grade_assigner.name)}` : ''}</div>`
            : '<div class="text-sm text-slate-500 mt-3 dark:text-slate-400">Оценка в журнал по этому бланку еще не поставлена.</div>';

        return `
            <div class="flex flex-wrap justify-between gap-4 items-start">
                <div>
                    <div class="text-sm uppercase tracking-[0.25em] text-slate-400 dark:text-slate-500">Оценка ученику</div>
                    <div class="text-lg font-semibold text-slate-900 mt-2 dark:text-white">Сохранить результат в журнал по дате</div>
                    ${currentGrade}
                </div>
                <button onclick="deleteCheckedBlankForm(${blankForm.id})" class="bg-rose-600 text-white px-4 py-3 rounded-2xl hover:bg-rose-500 transition text-sm">
                    Удалить работу
            ...
            <div class="grid md:grid-cols-[220px_220px_auto] gap-3 mt-5">
                <input id="assignedGradeValue-${blankForm.id}" type="text" value="${escapeAttribute(blankForm.assigned_grade_value || suggestAssignedGrade(blankForm, grade))}"
                       class="w-full px-4 py-3 rounded-2xl border border-slate-300 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
